"""Run side-by-side evaluation of the best ML model and analytical models for ethyl acetate."""
...
SOLVENT_PROPERTIES = {
    "Polarity (XLogP3)": 0.7,
    "Viscosity (cP)": 0.423,
    "Boiling Point (K)": 350.372,
    "Surface Tension (mN/m)": 24.0,
}
...
def save_scatter_plot(predictions_df: pd.DataFrame, output_path: Path) -> None:
    ...
    ax.set_title("Ethyl Acetate Experimental Comparison: ML vs Analytical Models")
...
def save_mobile_scatter_plot(predictions_df: pd.DataFrame, output_path: Path) -> None:
    ...
    ax.set_title("Ethyl Acetate Mobile Layer: Bayesian RF vs Landau--Levich")
    ax.grid(True, alpha=0.3)
    ax.legend()
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches="tight")

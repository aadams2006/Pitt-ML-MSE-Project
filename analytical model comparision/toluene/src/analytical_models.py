"""Analytical thickness models for the PDMS + toluene comparison workflow.
...
EXPERIMENT_SOLVENT = "toluene"
RELATIVE_EVAPORATION_REFERENCE = {
    "source": None,
    "relative_evaporation_buac_equals_1": None,
    "note": "No solvent-specific relative evaporation reference is currently recorded for toluene. E remains symbolic.",
}
...
def landau_levich_wet_mobile_layer_model(df: pd.DataFrame) -> pd.Series:
    """Predict dry mobile PDMS thickness from Landau--Levich entrainment.

    The wet solution film is converted with the PDMS volume fraction. Source
    workbooks identify concentration as g/L; the comparison CSV preserves the
    legacy ``Concentration (g/mL)`` header. This model is not fit to bonded
    thickness and is not registered in the bonded-thickness comparison.
    """

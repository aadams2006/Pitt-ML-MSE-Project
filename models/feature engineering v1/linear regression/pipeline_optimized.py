"""Linear Regression pipeline with OPTIMIZED hyperparameters."""
...
def load_agg_data(data_path: Path | None = None) -> pd.DataFrame:
    ...
    if data_path is None:
        data_path = Path(__file__).resolve().parents[1] / "data FE-V1" / "Hexane+PDMS-new.xlsx"
...
def train_and_evaluate(X_train, X_test, y_train, y_test, model_name: str = "Linear Regression") -> dict:
    """Train Linear Regression model and return evaluation metrics.
    
    Note: Linear Regression has no hyperparameters to optimize.
    This maintains the original implementation.
    """
    ...
    # No hyperparameters to optimize - Linear Regression is deterministic
